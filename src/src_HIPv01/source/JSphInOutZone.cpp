...
void JSphInOutZone::Reset(){
  ...
  #ifdef _WITHGPU
    if(PtzPosg)hipFree(PtzPosg);  PtzPosg=NULL;  // Replacing cudaFree with hipFree
    if(PtzAuxg)hipFree(PtzAuxg);  PtzAuxg=NULL;  // Replacing cudaFree with hipFree
  ...
  ,const JDsPartsInit *partsdata,JGaugeSystem *gaugesystem)
{
  ...
  //-Calculate initial number of new particles.
  ...
  if(ZsurfMode==ZSURF_Calculated){
    ...
    #ifdef _WITHGPU
    if(!Cpu){
      hipMalloc(&PtzPosg,NptInit);  // Replacing cudaMalloc with hipMalloc
      hipMemcpy(PtzPosg,PtzPos,sizeof(float3)*NptInit,hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
      hipMalloc(&PtzAuxg,NptInit);  // Replacing cudaMalloc with hipMalloc
