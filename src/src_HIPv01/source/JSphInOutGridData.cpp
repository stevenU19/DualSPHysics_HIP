...
void JSphInOutGridData::AllocateMemoryGpu(){
  if(Npt){
    hipMalloc(&Velx0g,Npt);  // Replacing fcuda::Malloc with hipMalloc
    hipMalloc(&Velx1g,Npt);  // Replacing fcuda::Malloc with hipMalloc
    hipMalloc(&SelVelxg,Npt);  // Replacing fcuda::Malloc with hipMalloc
    if(UseVelz){
      hipMalloc(&Velz0g,Npt);  // Replacing fcuda::Malloc with hipMalloc
      hipMalloc(&Velz1g,Npt);  // Replacing fcuda::Malloc with hipMalloc
      hipMalloc(&SelVelzg,Npt);  // Replacing fcuda::Malloc with hipMalloc
    }
  }
}
...
void JSphInOutGridData::FreeMemoryGpu(){
  if(Velx0g)  hipFree(Velx0g);    Velx0g  =NULL;  // Replacing cudaFree with hipFree
  if(Velx1g)  hipFree(Velx1g);    Velx1g  =NULL;  // Replacing cudaFree with hipFree
  if(SelVelxg)hipFree(SelVelxg);  SelVelxg=NULL;  // Replacing cudaFree with hipFree
  if(Velz0g)  hipFree(Velz0g);    Velz0g  =NULL;  // Replacing cudaFree with hipFree
  if(Velz1g)  hipFree(Velz1g);    Velz1g  =NULL;  // Replacing cudaFree with hipFree
  if(SelVelzg)hipFree(SelVelzg);  SelVelzg=NULL;  // Replacing cudaFree with hipFree
}
#endif
...
void JSphInOutGridData::ComputeTimeGpu(double t){
  ...
  if(TimeGpu!=t){
    ...
    //-Update data for time t0.
    if(CtVel0!=SelCt){
      CtVel0=SelCt;
      hipMemcpy(Velx0g,DataTimes[SelCt]->GetVelx(),sizeof(float)*Npt,hipMemcpyHostToDevice); // Changed cudaMemcpy to hipMemcpy
      if(UseVelz)hipMemcpy(Velz0g,DataTimes[SelCt]->GetVelz(),sizeof(float)*Npt,hipMemcpyHostToDevice); // Changed cudaMemcpy to hipMemcpy
    }
    //-Update data for time t1.
    unsigned selct1=(SelCt+1<nt? SelCt+1: SelCt);
    if(CtVel1!=selct1){
      CtVel1=selct1;
      hipMemcpy(Velx1g,DataTimes[selct1]->GetVelx(),sizeof(float)*Npt,hipMemcpyHostToDevice); // Changed cudaMemcpy to hipMemcpy
      if(UseVelz)hipMemcpy(Velz1g,DataTimes[selct1]->GetVelz(),sizeof(float)*Npt,hipMemcpyHostToDevice); // Changed cudaMemcpy to hipMemcpy
    }
    //-Update data for the requested time (t).
    if(t<=seltime || (t>=seltime && SelCt+1>=nt)){//-Copy data from DataTimes[SelCt].
      if(CtSelVel!=SelCt){
        CtSelVel=SelCt;
        hipMemcpy(SelVelxg,Velx0g,sizeof(float)*Npt,hipMemcpyDeviceToDevice); // Changed cudaMemcpy to hipMemcpy
        if(UseVelz)hipMemcpy(SelVelzg,Velz0g,sizeof(float)*Npt,hipMemcpyDeviceToDevice); // Changed cudaMemcpy to hipMemcpy
      }
    }
    else{ //-Interpolate data between DataTimes[SelCt] and DataTimes[SelCt+1].
      CtSelVel=UINT_MAX;
      ...
      cusphinout::InOutInterpolateTime(Npt,t,t0,t1,Velx0g,Velx1g,SelVelxg,Velz0g,Velz1g,SelVelzg); // No changes needed here, not CUDA-specific
    }
    TimeGpu=t;
  }
}
...
//==============================================================================
/// Interpolate velocity in time and Z-position of selected particles in a list.
//==============================================================================
...
void JSphInOutGridData::SaveDataVtk(std::string filename,int ctime)const{
  ...
  if(ctini >= CountTimes()) throw std::runtime_error("Number of DataTime is invalid.");
