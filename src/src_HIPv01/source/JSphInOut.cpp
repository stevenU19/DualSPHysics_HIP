...
#ifdef _WITHGPU
  #include "FunctionsHip.h"  // Replacing FunctionsCuda.h with FunctionsHip.h
...
void JSphInOut::AllocatePtMemoryGpu(unsigned ptcount){
  hipMalloc(&PtZoneg, ptcount);  // Replacing fcuda::Malloc with hipMalloc
  hipMalloc(&PtPosxyg, ptcount); // Replacing fcuda::Malloc with hipMalloc
  hipMalloc(&PtPoszg, ptcount);  // Replacing fcuda::Malloc with hipMalloc
  hipMalloc(&PtAuxDistg, ptcount);  // Replacing fcuda::Malloc with hipMalloc
}
...
void JSphInOut::FreePtMemoryGpu(){
  if(PtZoneg)   hipFree(PtZoneg);    PtZoneg = NULL;  // Replacing cudaFree with hipFree
  if(PtPosxyg)  hipFree(PtPosxyg);   PtPosxyg = NULL;  // Replacing cudaFree with hipFree
  if(PtPoszg)   hipFree(PtPoszg);    PtPoszg = NULL;  // Replacing cudaFree with hipFree
  if(PtAuxDistg)hipFree(PtAuxDistg); PtAuxDistg = NULL;  // Replacing cudaFree with hipFree
}
...
void JSphInOut::AllocateMemoryGpu(unsigned listsize){
  try{
    hipMalloc(&Planesg, ListSize);  // Replacing fcuda::Malloc with hipMalloc
    hipMalloc(&BoxLimitg, ListSize * 3);  // Replacing fcuda::Malloc with hipMalloc
    hipMalloc(&CfgZoneg, ListSize);  // Replacing fcuda::Malloc with hipMalloc
    hipMalloc(&CfgUpdateg, ListSize);  // Replacing fcuda::Malloc with hipMalloc
    hipMalloc(&Widthg, ListSize);  // Replacing fcuda::Malloc with hipMalloc
    hipMalloc(&DirDatag, ListSize);  // Replacing fcuda::Malloc with hipMalloc
    hipMalloc(&Zsurfg, ListSize);  // Replacing fcuda::Malloc with hipMalloc
  }
  catch(const std::bad_alloc){
    Run_Exceptioon("Could not allocate the requested memory.");
  }
...
void JSphInOut::FreeMemoryGpu(){
  if(Planesg)   hipFree(Planesg);     Planesg = NULL;  // Replacing cudaFree with hipFree
  if(BoxLimitg) hipFree(BoxLimitg);   BoxLimitg = NULL;  // Replacing cudaFree with hipFree
  if(CfgZoneg)  hipFree(CfgZoneg);    CfgZoneg = NULL;  // Replacing cudaFree with hipFree
  if(CfgUpdateg)hipFree(CfgUpdateg);  CfgUpdateg = NULL;  // Replacing cudaFree with hipFree
  if(Widthg)    hipFree(Widthg);      Widthg = NULL;  // Replacing cudaFree with hipFree
  if(DirDatag)  hipFree(DirDatag);    DirDatag = NULL;  // Replacing cudaFree with hipFree
  if(Zsurfg)    hipFree(Zsurfg);      Zsurfg = NULL;  // Replacing cudaFree with hipFree
}
#endif
  ...
  ,JGaugeSystem *gaugesystem,JNumexLib *nuxlib)
{
  ...
  UpdateZsurf(timestep, true);
    ...
    if(!Cpu){
      ...
      hipMemcpy(Planesg, Planes, sizeof(float4) * ListSize, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
      hipMemcpy(CfgZoneg, CfgZone, sizeof(byte) * ListSize, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
      hipMemcpy(CfgUpdateg, CfgUpdate, sizeof(byte) * ListSize, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
      hipMemcpy(Widthg, Width, sizeof(float) * ListSize, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
      hipMemcpy(DirDatag, DirData, sizeof(float3) * ListSize, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
      ...
      if(UseBoxLimit){
        ...
        hipMemcpy(BoxLimitg, boxlimit, sizeof(float2) * ListSize * 3, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
  ...
  if(RefillAdvanced || CalculatedZsurf){
    ...
    for(unsigned ci = 0; ci < ListSize; ci++){
      ...
      if(!Cpu){
        ...
        //-Copies data to GPU memory.
        hipMemcpy(PtZoneg, PtZone, sizeof(byte) * PtCount, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
        hipMemcpy(PtPosxyg, pxy, sizeof(double2) * PtCount, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
        hipMemcpy(PtPoszg, pz, sizeof(double) * PtCount, hipMemcpyHostToDevice);  // Replacing cudaMemcpy with hipMemcpy
...
bool JSphInOut::UpdateZsurf(double timestep, bool full) {
    bool modified = full;
    for (unsigned ci = 0; ci < ListSize; ci++) {
        if (full || List[ci]->GetVariableZsurf() || List[ci]->GetCalculatedZsurf()) {
            modified |= List[ci]->UpdateZsurf(timestep, full, Zsurf[ci]);
        }
    }
    
    #ifdef _WITHGPU
        // Reemplazamos cudaMemcpy con hipMemcpy
        if (modified && !Cpu) {
            hipMemcpy(Zsurfg, Zsurf, sizeof(float) * ListSize, hipMemcpyHostToDevice);
        }
    #endif
