...
//==============================================================================
void JGaugeItem::RunExceptionHip(const std::string &srcfile,int srcline
...
//==============================================================================
/// Checks HIP error and throws exception if any.
  ...
  ,const typecode *code,const unsigned *idp,const float4 *velrhop,float3 *aux)
{
  ...
  //-Start measure.
  tfloat3 ptvel = TFloat3(0);
  const bool ptout = PointIsOut(Point.x, Point.y, Point.z); //-Verify that the point is within domain boundaries.
  
  if (!ptout) {
    // Utiliza la función HIP en lugar de CUDA.
    cugauge::Interaction_GaugeVel(CSP, dvd, Point, posxy, posz, code, velrhop, aux);
    // Cambiado cudaMemcpy a hipMemcpy.
    hipMemcpy(&ptvel, aux, sizeof(float3), hipMemcpyDeviceToHost);
    CheckHipError(__FILE__, __LINE__, ClassName, "CalculeGpu", "Failed in velocity calculation.");
  }
...
{
  SetTimeStep(timestep);
  //-Look for the surface water level using GPU.
  cugauge::Interaction_GaugeSwl(CSP, dvd, Point0, PointDir, PointNp, MassLimit, posxy, posz, code, velrhop, aux);

  // Recupera los resultados del dispositivo.
  tfloat3 ptsurf = TFloat3(0);
  hipMemcpy(&ptsurf, aux, sizeof(float3), hipMemcpyDeviceToHost);
  CheckHipError(__FILE__, __LINE__, ClassName, "CalculeGpu", "Failed in SWL calculation.");
  ...
  ,const typecode *code,const unsigned *idp,const float4 *velrhop,float3 *aux)
{
  ...
  //-Start measure using HIP.
  cugauge::Interaction_GaugeMaxz(Point0, maxdist2, dvd, cxini, cxfin, yini, yfin, zini, zfin, posxy, posz, code, aux);

  // Recupera el resultado desde la GPU a la CPU.
  tfloat3 ptsurf = TFloat3(0);
  hipMemcpy(&ptsurf, aux, sizeof(float3), hipMemcpyDeviceToHost);  // Cambiado de cudaMemcpy a hipMemcpy.
  CheckHipError(__FILE__, __LINE__, ClassName, "CalculeGpu", "Failed in MaxZ calculation.");
...
#ifdef _WITHGPU
  if (!Cpu) hipMalloc(&PartAceg, Count * sizeof(tfloat3));  // Cambiado de cudaMalloc a hipMalloc.
  unsigned saux = curedus::GetAuxSize_ReduSumFloat3(Count);
  if (!Cpu) hipMalloc(&Auxg, saux);  // Cambiado de cudaMalloc a hipMalloc.
#endif
}
...
#ifdef _WITHGPU
  if (PartAceg) hipFree(PartAceg);  // Cambiado de cudaFree a hipFree.
  PartAceg = NULL;
  
  if (Auxg) hipFree(Auxg);  // Cambiado de cudaFree a hipFree.
  Auxg = NULL;
#endif
  ...
  ,const typecode *code,const unsigned *idp,const float4 *velrhop,float3 *aux)
{
  ...
  hipMemset(PartAceg, 0, sizeof(float3) * Count);  // Cambiado de cudaMemset a hipMemset.
  const int n = int(TypeParts == TpPartFixed || TypeParts == TpPartMoving ? npbok : np);

  //-Calcula la aceleración en partículas límite seleccionadas usando HIP.
  cugauge::Interaction_GaugeForce(CSP, dvd, n, IdBegin, Code, posxy, posz, code, idp, velrhop, PartAceg);
  ...
  CheckHipError(__FILE__, __LINE__, ClassName, "CalculeGpu", "Failed in Force calculation.");
