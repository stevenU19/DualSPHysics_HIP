...
void JCellDivGpuSingle::PreSort(const unsigned *dcellg,const typecode *codeg){
  if(DivideFull)hipdiv::PreSortFull(Nptot,DomCellCode,dcellg,codeg,CellDomainMin,TUint3(Ncx,Ncy,Ncz),CellPart,SortPart); // Cambio cudiv -> hipdiv
  else hipdiv::PreSortFluid(Npf1,Npb1,DomCellCode,dcellg,codeg,CellDomainMin,TUint3(Ncx,Ncy,Ncz),CellPart,SortPart); // Cambio cudiv -> hipdiv
}
  ...
  ,const unsigned *dcellg,const typecode *codeg,TimersGpu timers,const double2 *posxy,const double *posz,const unsigned *idp)
{
  ...
  if(DivideFull)hipdiv::Sort(CellPart,SortPart,Nptot,Stable); // Cambio cudiv -> hipdiv
  else hipdiv::Sort(CellPart+Npb1,SortPart+Npb1,Nptot-Npb1,Stable); // Cambio cudiv -> hipdiv
  ...
  hipdiv::CalcBeginEndCell(DivideFull,Nptot,Npb1,unsigned(SizeBeginEndCell(Nct)),BoxFluid,CellPart,BeginEndCell); // Cambio cudiv -> hipdiv
  ...
  // Modificación: Cambiar Check_CudaErroor a Check_HipError
  Check_HipError("Error in NL construction.");
