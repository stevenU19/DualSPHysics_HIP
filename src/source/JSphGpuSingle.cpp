...
void JSphGpuSingle::ConfigDomain(){
  //-Computes the number of particles. // calcula el numero de particulas.
  Np=PartsLoaded->GetCount(); Npb=CaseNpb; NpbOk=Npb;
  //-Allocates memory for arrays with fixed size (motion and floating bodies). /Asigna memoria para matrices con tamaño fijo (movimiento y cuerpos flotantes).
  AllocGpuMemoryFixed();
  //-Allocates GPU memory for particles. // Asignacion de memoria en GPU para particulas
  AllocGpuMemoryParticles(Np,0);
  //-Allocates memory on the CPU. // Asignacion de memoria en CPU
  AllocCpuMemoryFixed();
  ...
  //-Configure cell division. / Configuracion de division de celdas
  ConfigCellDivision();
...
void JSphGpuSingle::RunPeriodic(){
  ...
  cusph::PeriodicIgnore(Np,Codeg); // Cambiado a hip::PeriodicIgnore para HIP
    ...
    for(unsigned cper=0;cper<3;cper++)if((cper==0 && PeriX) || (cper==1 && PeriY) || (cper==2 && PeriZ)){
      ...
      for(unsigned cblock=0;cblock<2;cblock++){//-0:new periodic particles, 1:original periodic particles
        ...
        while(run && num2){
          ...
          unsigned count=cusph::PeriodicMakeList(num2,pini2,Stable,nmax,Map_PosMin,Map_PosMax,perinc,Posxyg,Poszg,Codeg,listpg); // Cambiado de cusph::PeriodicMakeList a hip::PeriodicMakeList para HIP
          ...
          if(count>nmax || !CheckGpuParticlesSize(count+Np)){
            ...
            if(TStep==STEP_Verlet)cusph::PeriodicDuplicateVerlet(count,Np,DomCells,perinc,listpg,Idpg,Codeg,Dcellg,Posxyg,Poszg,Velrhopg,SpsTaug,VelrhopM1g); // Cambio de cusph::PeriodicDuplicateVerlet a hip::PeriodicDuplicateVerlet para HIP
            if(TStep==STEP_Symplectic){
              ...
              cusph::PeriodicDuplicateSymplectic(count,Np,DomCells,perinc,listpg,Idpg,Codeg,Dcellg,Posxyg,Poszg,Velrhopg,SpsTaug,PosxyPreg,PoszPreg,VelrhopPreg); // Cambio de cusph::PeriodicDuplicateSymplectic a hip::PeriodicDuplicateSymplectic
            }
            if(UseNormals)cusph::PeriodicDuplicateNormals(count,Np,listpg,BoundNormalg,MotionVelg);  //<vs_mddbc> // Cambiado a hip::PeriodicDuplicateNormals
  ...
  Check_HipError("Failed in creation of periodic particles."); // Cambio de Check_CudaErroor a Check_HipError
  TmgStop(Timers,TMG_SuPeriodic);
}
...
void JSphGpuSingle::RunCellDivide(bool updateperiodic){
  ...
  TmgStart(Timers,TMG_NlSortData);
  {
    ...
    // Cambio: Se cambió la llamada de cudaMemcpy a hipMemcpy
  ...
  // Cambio: Se cambia la llamada a cudaMemcpy a hipMemcpy para actualizar PosCellg[]
  hipError_t status = hipMemcpy(PosCellg, Posxyg, sizeof(double2) * Np, hipMemcpyDeviceToDevice);
  ...
  //-Collect position of floating particles.
...
void JSphGpuSingle::Interaction_Forces(TpInterStep interstep){
  if(TBoundary==BC_MDBC && (MdbcCorrector || interstep!=INTERSTEP_SymCorrector))MdbcBoundCorrection();
  InterStep=interstep;
  ...
  // Cambio: Se cambió la llamada a cusph::Interaction_Forces en CUDA a HIP
  ...
  //-Applies DDT.
  if(Deltag)cusph::AddDelta(Np-Npb,Deltag+Npb,Arg+Npb);

  Check_HipError("Failed while executing kernels of interaction.");  // Cambio: Se cambió Check_CudaError a Check_HipError
  ...
  Check_HipError("Failed in reduction of viscdt.");
...
double JSphGpuSingle::ComputeAceMax(float *auxmem){
  ...
  if(!check)cusph::ComputeAceMod(npf,Aceg+Npb,auxmem);
  else cusph::ComputeAceMod(npf,Codeg+Npb,Aceg+Npb,auxmem);
...
double JSphGpuSingle::ComputeStep_Ver(){
  if(BoundCorr)BoundCorrectionData();
  Interaction_Forces(INTERSTEP_Verlet);
  const double dt=DtVariable(true);
  if(CaseNmoving)CalcMotion(dt);
  DemDtForce=dt;
  if(Shifting)RunShifting(dt);
  ComputeVerlet(dt);
  if(CaseNfloat)RunFloating(dt,false);
  PosInteraction_Forces();
  if(Damping)RunDamping(dt,Np,Npb,Posxyg,Poszg,Codeg,Velrhopg);
  if(RelaxZones)RunRelaxZone(dt);
  return(dt);
}
...
double JSphGpuSingle::ComputeStep_Sym(){
  ...
  DemDtForce=dt*0.5f;
  if(BoundCorr)BoundCorrectionData();
  Interaction_Forces(INTERSTEP_SymPredictor);
  const double ddt_p=DtVariable(false);
  if(Shifting)RunShifting(dt*.5);
  ComputeSymplecticPre(dt);
  if(CaseNfloat)RunFloating(dt*.5,true);
  PosInteraction_Forces();
  DemDtForce=dt;
  RunCellDivide(true);
  Interaction_Forces(INTERSTEP_SymCorrector);
  const double ddt_c=DtVariable(true);
  if(Shifting)RunShifting(dt);
  ...
  SymplecticDtPre=min(ddt_p,ddt_c);
  return(dt);
}
...
void JSphGpuSingle::UpdateFtObjs(){
  if(FtCount && FtObjsOutdated){
    ...
    hipMemcpy(fvel,FtoVelg,sizeof(float3) *FtCount,hipMemcpyDeviceToHost);
    hipMemcpy(fome,FtoOmegag,sizeof(float3) *FtCount,hipMemcpyDeviceToHost);
...
void JSphGpuSingle::FtApplyImposedVel(float3 *ftoforcesresg)const{
  ...
  for(unsigned cf=0;cf<FtCount;cf++)if(!FtObjs[cf].usechrono && (FtLinearVel[cf]!=NULL || FtAngularVel[cf]!=NULL)){
    ...
    if(!ftoforcesresc && (v1!=TFloat3(FLT_MAX) || v2!=TFloat3(FLT_MAX))){
      // Copia los datos en la memoria de la GPU a la memoria de la CPU
  ...
  // Actualiza los datos en la memoria de la GPU
...
void JSphGpuSingle::RunFloating(double dt,bool predictor){
  if(TimeStep>=FtPause){
    ...
    // Inicializa fuerzas de los objetos flotantes.
    
    // Cambio: cudaMemset -> hipMemset
    hipMemset(FtoForcesg,0,sizeof(StFtoForces)*FtCount);

    // Añade aceleraciones de ForcePoints y Moorings.
    if(ForcePoints){
      // Inicializa fuerzas de los objetos flotantes.
      ...
      // Copia los datos a la memoria GPU.
      
      // Cambio: cudaMemcpy -> hipMemcpy
      hipMemcpy(FtoForcesg,ftoforces,sizeof(StFtoForces)*FtCount,hipMemcpyHostToDevice);
    }

    // Copia las fuerzas externas al array FtoExtForcesg.
    if(FtLinearForce!=NULL)FtCopyExternalForces();
    // Calcula la suma de fuerzas (face,fomegaace) comenzando con las partículas flotantes en ftoforcessum[].
    cusph::FtCalcForcesSum(PeriActive!=0,FtCount,FtoDatpg,FtoCenterg,FtRidpg,Posxyg,Poszg,Aceg,FtoForcesSumg);
    // Añade aceleración de partículas y fuerzas externas a FtoForcesg[].
    ...
    // Guarda face y fomegace para depuración.
    if(SaveFtAce){
      StFtoForces *ftoforces=(StFtoForces *)FtoAuxFloat9;
    ...
    // Ejecuta flotadores con la biblioteca Chrono.
    if(ChronoObjects){      
      ...
      // Carga los datos calculados por Chrono.
      tdouble3* ftocenter=FtoAuxDouble6;
      
      // Cambio: cudaMemcpy -> hipMemcpy
      hipMemcpy(ftocenter,FtoCenterResg,sizeof(tdouble3)*FtCount,hipMemcpyDeviceToHost);
      hipMemcpy(ftoforces,FtoForcesResg,sizeof(tfloat3)*FtCount*2,hipMemcpyDeviceToHost);
      for(unsigned cf=0;cf<FtCount;cf++)if(FtObjs[cf].usechrono)
        ChronoObjects->GetFtData(FtObjs[cf].mkbound,ftocenter[cf],ftoforces[cf*2+1],ftoforces[cf*2]);
    ...
    // Aplica movimiento alrededor de los objetos flotantes.
    ...
    // Almacena los datos de flotadores.
    if(!predictor){
      FtObjsOutdated=true;
    }

    // Actualiza los datos de puntos en FtForces y calcula los datos de movimiento de los flotadores afectados.
