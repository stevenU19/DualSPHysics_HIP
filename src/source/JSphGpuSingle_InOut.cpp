...
void JSphGpuSingle::InOutCheckProximity(unsigned newnp){
  if(Np && newnp){
    ...
    hipMemcpy(Codeg,Code,sizeof(typecode)*Np,hipMemcpyHostToDevice); // Cambio cudaMemcpy -> hipMemcpy
  }
}
...
void JSphGpuSingle::InOutInit(double timestepini){
  ...
  hipMemcpy(Idpg    +Np,Idp    ,sizeof(unsigned)*newnp,hipMemcpyHostToDevice); // Cambio cudaMemcpy -> hipMemcpy
  hipMemcpy(Codeg   +Np,Code   ,sizeof(typecode)*newnp,hipMemcpyHostToDevice); // Cambio cudaMemcpy -> hipMemcpy
  hipMemcpy(Posxyg  +Np,Posxy  ,sizeof(double2) *newnp,hipMemcpyHostToDevice); // Cambio cudaMemcpy -> hipMemcpy
  hipMemcpy(Poszg   +Np,Posz   ,sizeof(double)  *newnp,hipMemcpyHostToDevice); // Cambio cudaMemcpy -> hipMemcpy
  hipMemcpy(Velrhopg+Np,Velrhop,sizeof(float4)  *newnp,hipMemcpyHostToDevice); // Cambio cudaMemcpy -> hipMemcpy
  ...
  if(SpsTaug)hipMemset(SpsTaug+Np,0,sizeof(tsymatrix3f)*newnp); // Cambio cudaMemset -> hipMemset
