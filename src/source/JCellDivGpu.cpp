...
void JCellDivGpu::FreeMemoryNct(){
  hipFree(BeginEndCell); BeginEndCell=NULL; // Cambio cudaFree -> hipFree
  MemAllocGpuNct=0;
  BoundDivideOk=false;
}
...
void JCellDivGpu::FreeMemoryAll(){
  FreeMemoryNct();
  hipFree(CellPart);  CellPart=NULL;  // Cambio cudaFree -> hipFree
  hipFree(SortPart);  SortPart=NULL;  // Cambio cudaFree -> hipFree
  hipFree(AuxMem);    AuxMem=NULL;    // Cambio cudaFree -> hipFree
  MemAllocGpuNp=0;
  BoundDivideOk=false;
}
...
void JCellDivGpu::AllocMemoryNp(ullong np){
  ...
  hipMalloc((void**)&CellPart,m); MemAllocGpuNp+=m;  // Cambio cudaMalloc -> hipMalloc
  hipMalloc((void**)&SortPart,m); MemAllocGpuNp+=m;  // Cambio cudaMalloc -> hipMalloc
  SizeAuxMem=hipdiv::LimitsPosSize(SizeNp); // Cambio cudiv -> hipdiv
  m=sizeof(float)*SizeAuxMem;
  hipMalloc((void**)&AuxMem,m);   MemAllocGpuNp+=m;  // Cambio cudaMalloc -> hipMalloc
  //-Checks allocated memory.
  //-Comprueba reserva de memoria.
  hipError_t hiperr=hipGetLastError(); // Cambio cudaError_t -> hipError_t y cudaGetLastError -> hipGetLastError
  if(hiperr!=hipSuccess){
    Run_ExceptionHip(hiperr,fun::PrintStr("Failed GPU memory allocation of %.1f MB for %u particles.",double(MemAllocGpuNp)/(1024*1024),SizeNp));
  }
...
void JCellDivGpu::AllocMemoryNct(ullong nct){
  ...
  hipMalloc((void**)&BeginEndCell,m); MemAllocGpuNct+=m;  // Cambio cudaMalloc -> hipMalloc
  //-Checks allocated memory.
  //-Comprueba reserva de memoria.
  hipError_t hiperr=hipGetLastError(); // Cambio cudaError_t -> hipError_t y cudaGetLastError -> hipGetLastError
  if(hiperr!=hipSuccess){
    Run_ExceptionHip(hiperr,fun::PrintStr("Failed GPU memory allocation of %.1f MB for %u cells.",double(MemAllocGpuNct)/(1024*1024),SizeNct));
  }
...
{
  tuint3 cmin,cmax;
  hipdiv::LimitsCell(n,pini,DomCellCode,dcellg,codeg,(unsigned*)AuxMem,cmin,cmax,Log); // Cambio cudiv -> hipdiv
  cellmin=(cmin.x>cmax.x? DomCells: cmin);
  cellmax=(cmin.x>cmax.x? TUint3(0): cmax);
  if(n2){
    hipdiv::LimitsCell(n2,pini2,DomCellCode,dcellg,codeg,(unsigned*)AuxMem,cmin,cmax,Log); // Cambio cudiv -> hipdiv
...
void JCellDivGpu::CellBeginEnd(unsigned cell,unsigned ndata,unsigned* data)const{
  hipMemcpy(data,BeginEndCell+cell,sizeof(int)*ndata,hipMemcpyDeviceToHost); // Cambio cudaMemcpy -> hipMemcpy
}
...
int2 JCellDivGpu::CellBeginEnd(unsigned cell)const{
  int2 v;
  hipMemcpy(&v,BeginEndCell+cell,sizeof(int2),hipMemcpyDeviceToHost); // Cambio cudaMemcpy -> hipMemcpy
  return(v);
}
  ...
  ,unsigned *idp2,typecode *code2,unsigned *dcell2,double2 *posxy2,double *posz2,float4 *velrhop2)
{
  ...
  hipdiv::SortDataParticles(Nptot,pini,SortPart,idp,code,dcell,posxy,posz,velrhop,idp2,code2,dcell2,posxy2,posz2,velrhop2); // Cambio cudiv -> hipdiv
}
...
void JCellDivGpu::SortDataArrays(const float4 *a,float4 *a2){
  const unsigned pini=(DivideFull? 0: NpbFinal);
  hipdiv::SortDataParticles(Nptot,pini,SortPart,a,a2); // Cambio cudiv -> hipdiv
}
...
void JCellDivGpu::SortDataArrays(const float *a,const float *b,float *a2,float *b2){
  const unsigned pini=(DivideFull? 0: NpbFinal);
  hipdiv::SortDataParticles(Nptot,pini,SortPart,a,b,a2,b2); // Cambio cudiv -> hipdiv
}
...
void JCellDivGpu::SortDataArrays(const double2 *a,const double *b,const float4 *c,double2 *a2,double *b2,float4 *c2){
  const unsigned pini=(DivideFull? 0: NpbFinal);
  hipdiv::SortDataParticles(Nptot,pini,SortPart,a,b,c,a2,b2,c2); // Cambio cudiv -> hipdiv
}
...
void JCellDivGpu::DgSaveVktRange(std::string file, unsigned pini, unsigned pfin, const unsigned *idpg, const float3 *posg) const {
    ...
    // Traducción de cudaMemcpy a hipMemcpy
    ...
    // Manteniendo el resto del código igual, ya que no está relacionado con CUDA
