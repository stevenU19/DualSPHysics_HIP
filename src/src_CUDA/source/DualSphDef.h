...
//-Defines AVAILABLE_MGPU when this feature is compiled. / Define multi GPU cuando se compila esta funcion.
#ifdef _WITHMGPU
...
///Types of interaction step.// Tipos de pasos de interacción.
typedef enum{ 
  INTERSTEP_None=0,         
  INTERSTEP_Verlet=1,       ///<Interaction to compute forces using the Verlet algorithm. / Omteraccion para computar fuerzas usando el algoritmo verlet
  INTERSTEP_SymPredictor=2, ///<Interaction to compute forces using the Symplectic algorithm (predictor step). / computa fuerzas usando el algoritmo sympletic
  INTERSTEP_SymCorrector=3  ///<Interaction to compute forces using the Symplectic algorithm (corrector step). / Interaccion computando fuerzas usando el algoritmo simpletic (paso corrector)
}TpInterStep;
