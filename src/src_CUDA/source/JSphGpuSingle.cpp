...
void JSphGpuSingle::Interaction_Forces(TpInterStep interstep){
  ...
  const StInterParmsg parms=StrInterParmsg(
    Simulate2D, // Activa la simulacion 2D
    Symmetry, //<vs_syymmetry> Simetria
    TKernel, //Tipo de kernel
    FtMode, //Modo de interaccion para flotantes
    lamsps, //Lamps
    TDensity, //Densidad
    ShiftingMode, //Tipos de termino de difusion de la densidad
    Visco*ViscoBoundFactor, //viscob
    Visco, // Viscof
    bsbound, //Bsbound
    bsfluid, //Bsfluid
    Np, //vnp
...
void JSphGpuSingle::FtApplyImposedVel(float3 *ftoforcesresg)const{
  ...
  for(unsigned cf=0;cf<FtCount;cf++)if(!FtObjs[cf].usechrono && (FtLinearVel[cf]!=NULL || FtAngularVel[cf]!=NULL)){
    ...
    if(!ftoforcesresc && (v1!=TFloat3(FLT_MAX) || v2!=TFloat3(FLT_MAX))){
      //-Copies data on GPU memory to CPU memory. // Copia los datos en la memoria de la GPU a la memoria de la CPU
