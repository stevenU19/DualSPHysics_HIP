...
void JArraysGpuSize::FreeMemory(){
  for(unsigned c=0;c<Count;c++)if(Pointers[c]){ hipFree(Pointers[c]); Pointers[c]=NULL; } // Cambio cudaFree -> hipFree
  CountUsed=Count=0;
}
...
void JArraysGpuSize::SetArrayCount(unsigned count){
  ...
  if(ArraySize){
    if(Count<count){//-Genera nuevos arrays. //-Generates new arrays
      for(unsigned c=Count;c<count;c++)hipMalloc((void**)(Pointers+c),ElementSize*ArraySize); // Cambio cudaMalloc -> hipMalloc
      Check_HipError("Failed GPU memory allocation.");
    }
    if(Count>count){//-Libera arrays. //-Frees arrays
      for(unsigned c=count;c<Count;c++){ hipFree(Pointers[c]); Pointers[c]=NULL; } // Cambio cudaFree -> hipFree
    }
  }
  Count=count;
