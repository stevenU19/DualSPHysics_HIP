    ...
    for(unsigned cper=0;cper<3;cper++)if((cper==0 && PeriX) || (cper==1 && PeriY) || (cper==2 && PeriZ)){
      ...
      for(unsigned cblock=0;cblock<2;cblock++){//-0:new periodic particles, 1:original periodic particles
        ...
        while(run && num2){
          ...
          unsigned count=hip::PeriodicMakeList(num2,pini2,Stable,nmax,Map_PosMin,Map_PosMax,perinc,Posxyg,Poszg,Codeg,listpg); // Cambiado de cusph::PeriodicMakeList a hip::PeriodicMakeList para HIP
          ...
          if(count>nmax || !CheckGpuParticlesSize(count+Np)){
            ...
            if(TStep==STEP_Verlet)hip::PeriodicDuplicateVerlet(count,Np,DomCells,perinc,listpg,Idpg,Codeg,Dcellg,Posxyg,Poszg,Velrhopg,SpsTaug,VelrhopM1g); // Cambio de cusph::PeriodicDuplicateVerlet a hip::PeriodicDuplicateVerlet para HIP
            if(TStep==STEP_Symplectic){
              ...
              hip::PeriodicDuplicateSymplectic(count,Np,DomCells,perinc,listpg,Idpg,Codeg,Dcellg,Posxyg,Poszg,Velrhopg,SpsTaug,PosxyPreg,PoszPreg,VelrhopPreg); // Cambio de cusph::PeriodicDuplicateSymplectic a hip::PeriodicDuplicateSymplectic
            }
            if(UseNormals)hip::PeriodicDuplicateNormals(count,Np,listpg,BoundNormalg,MotionVelg);  //<vs_mddbc> // Cambiado a hip::PeriodicDuplicateNormals
