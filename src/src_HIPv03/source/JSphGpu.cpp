...
JSphGpu::JSphGpu(bool withmpi):JSph(false,false,withmpi),DivAxis(MGDIV_None){
  ...
  CellDiv=NULL;
  FtoAuxDouble6=NULL; FtoAuxFloat9=NULL; //-Calculates forces on floating bodies. / -Calcula las fuerzas sobre los cuerpos flotantes.
...
JSphGpu::~JSphGpu(){
  ...
  hipDeviceReset(); // Cambio: cudaDeviceReset -> hipDeviceReset
}

//==============================================================================
/// Throws exception related to a HIP error from a static method. / Lanza una excepción relacionada con un error HIP de un método estático.
//==============================================================================
void JSphGpu::RunExceptionHipStatic(const std::string &srcfile,int srcline
  ,const std::string &method
  ,hipError_t hiperr,std::string msg) // Cambio: cudaError_t -> hipError_t
{
  msg=msg+fun::PrintStr(" (HIP error %d (%s)).\n",hiperr,hipGetErrorString(hiperr)); // Cambio: cudaGetErrorString -> hipGetErrorString
  throw JException(srcfile,srcline,"JSphGpu",method,msg,"");
}

//==============================================================================
/// Checks HIP error and throws exception from a static method.
//==============================================================================
void JSphGpu::CheckHipErrorStatic(const std::string &srcfile,int srcline
  ,const std::string &method,std::string msg)
{
  const hipError_t hiperr=hipGetLastError(); // Cambio: cudaError_t -> hipError_t, cudaGetLastError -> hipGetLastError
  if(hiperr!=hipSuccess)RunExceptionHipStatic(srcfile,srcline,method,hiperr,msg);
}
...
void JSphGpu::RunExceptionHipStatic(const std::string &srcfile,int srcline
  ...
  ,hipError_t hiperr,std::string msg)const // Cambio: cudaError_t -> hipError_t
{
  msg=msg+fun::PrintStr(" (HIP error %d (%s)).\n",hiperr,hipGetErrorString(hiperr)); // Cambio: cudaGetErrorString -> hipGetErrorString
...
/// Comprueba error de HIP y lanza excepcion si lo hubiera.
//==============================================================================
void JSphGpu::CheckHipError(const std::string &srcfile,int srcline
  ,const std::string &classname,const std::string &method
  ,std::string msg)const
{
  hipError_t hiperr=hipGetLastError(); // Cambio: cudaError_t -> hipError_t, cudaGetLastError -> hipGetLastError
...
void JSphGpu::FreeGpuMemoryFixed(){
  MemGpuFixed=0;
  if(RidpMoveg)         hipFree(RidpMoveg);          RidpMoveg=NULL; // Cambio: cudaFree -> hipFree
  ...
  if(FtoForcesSumg)     hipFree(FtoForcesSumg);      FtoForcesSumg=NULL;
  if(FtoExtForcesg)     hipFree(FtoExtForcesg);      FtoExtForcesg=NULL;
  ...
  if(FtoVelg)           hipFree(FtoVelg);            FtoVelg=NULL;
  if(FtoOmegag)         hipFree(FtoOmegag);          FtoOmegag=NULL;
...
void JSphGpu::AllocGpuMemoryFixed(){
  ...
  if(CaseNmoving){
    ...
    hipMalloc((void**)&RidpMoveg,m);   MemGpuFixed+=m; // Cambio: cudaMalloc -> hipMalloc
  }
  //-Allocates memory for floating bodies. / Asignacion de memoria para cuerpos flotantes
  if(CaseNfloat){
    size_t m=0;
    ...
    m=sizeof(float3)  *FtCount*2;   hipMalloc((void**)&FtoForcesSumg     ,m);  MemGpuFixed+=m;
    m=sizeof(float3)  *FtCount*2;   hipMalloc((void**)&FtoExtForcesg     ,m);  MemGpuFixed+=m;
    ...
    m=sizeof(float3)  *FtCount;     hipMalloc((void**)&FtoVelg           ,m);  MemGpuFixed+=m;
    m=sizeof(float3)  *FtCount;     hipMalloc((void**)&FtoOmegag         ,m);  MemGpuFixed+=m;
...
void JSphGpu::AllocGpuMemoryParticles(unsigned np,float over){
  ...
  Check_HipError("Failed GPU memory allocation."); // Cambio: Check_CudaErroor -> Check_HipError
}

//==============================================================================
/// Resizes space in GPU memory for particles. // Redimensiona el espacio en la memoria de la GPU para las partículas.
//==============================================================================
void JSphGpu::ResizeGpuMemoryParticles(unsigned npnew){
  npnew=npnew+PARTICLES_OVERMEMORY_MIN;
  //-Saves current data from GPU. // Guarda los datos actuales de la GPU
...
//==============================================================================
/// Saves a GPU array in CPU memory. // Guarda una matriz de la GPU en la memoria de la CPU
//==============================================================================
template<class T> T* JSphGpu::TSaveArrayGpu(unsigned np,const T *datasrc)const{
  ...
  if(datasrc){
    ...
    hipMemcpy(data,datasrc,sizeof(T)*np,hipMemcpyDeviceToHost); // Cambio: cudaMemcpy -> hipMemcpy
  }
  return(data);
}

//==============================================================================
/// Restores a GPU array (generic) from CPU memory.  // Restaura una matriz de la GPU (genérica) desde la memoria de la CPU
//==============================================================================
template<class T> void JSphGpu::TRestoreArrayGpu(unsigned np,T *data,T *datanew)const{
  if(data&&datanew)hipMemcpy(datanew,data,sizeof(T)*np,hipMemcpyHostToDevice); // Cambio: cudaMemcpy -> hipMemcpy
  delete[] data;
}
...
void JSphGpu::ConstantDataUp(){
  ...
  Check_HipError("Failed copying constants to GPU."); // Cambio: Check_CudaErroor -> Check_HipError
}
...
void JSphGpu::ParticlesDataUp(unsigned n,const tfloat3 *boundnormal){
  hipMemcpy(Idpg    ,Idp    ,sizeof(unsigned)*n,hipMemcpyHostToDevice); // Cambio: cudaMemcpy -> hipMemcpy
  ...
  if(UseNormals)hipMemcpy(BoundNormalg,boundnormal,sizeof(float3)*n,hipMemcpyHostToDevice); //<vs_mddbc>
  Check_HipError("Failed copying data to GPU."); // Cambio: Check_CudaErroor -> Check_HipError
}

//==============================================================================
/// Recovers particle data from the GPU and returns the particle number that
/// are less than n if the periodic particles are removed.
...
unsigned JSphGpu::ParticlesDataDown(unsigned n,unsigned pini,bool code,bool onlynormal){
  unsigned num=n;
  hipMemcpy(Idp    ,Idpg    +pini,sizeof(unsigned)*n,hipMemcpyDeviceToHost); // Cambio: cudaMemcpy -> hipMemcpy
  ...
  Check_HipError("Failed copying data from GPU."); // Cambio: Check_CudaErroor -> Check_HipError
...
//==============================================================================
/// Initialises HIP device.
/// Inicializa dispositivo HIP.
//==============================================================================
void JSphGpu::SelecDevice(int gpuid){
  Log->Print("[Select HIP Device]"); // Cambio: CUDA -> HIP
  //-Get and show GPU information.
  vector<string> gpuinfo;
  vector<fhip::StGpuInfo> gpuprops;
  const int devcount=fhip::GetHipDevicesInfo(&gpuinfo,&gpuprops); // Cambio: GetCudaDevicesInfo -> GetHipDevicesInfo
  Log->Print(gpuinfo);
  Log->Print(" ");
  //-GPU selection.
  GpuSelect=-1;
  if(devcount){
    if(gpuid>=0)hipSetDevice(gpuid); // Cambio: cudaSetDevice -> hipSetDevice
    else{
      unsigned *ptr=NULL;
      hipMalloc((void**)&ptr,sizeof(unsigned)*100); // Cambio: cudaMalloc -> hipMalloc
      hipFree(ptr); // Cambio: cudaFree -> hipFree
    }
    hipDeviceProp_t devp; // Cambio: cudaDeviceProp -> hipDeviceProp_t
    int dev;
    hipGetDevice(&dev); // Cambio: cudaGetDevice -> hipGetDevice
    hipGetDeviceProperties(&devp,dev); // Cambio: cudaGetDeviceProperties -> hipGetDeviceProperties
    GpuSelect=dev;
    GpuName=devp.name;
  ...
  else Run_Exceptioon("There are no available HIP devices."); // Cambio: CUDA -> HIP
}

//==============================================================================
/// Configures BlockSize for main interaction HIP kernels. // Configura el BlockSize para los kernels HIP de interacción principal. 
//==============================================================================
  ...
  if(CellMode==CELLMODE_Full || CellMode==CELLMODE_Half){
    ...
    memset(&divdatag,0,sizeof(StDivDataGpu));
      ...
      const StInterParmsg parms=StrInterParmsg(
        Simulate2D,//Activa la simulacion 2D: simulate2d_
        Symmetry,  //<vs_syymmetry>: symmetry_
        TKernel, //tipo de Kernel:tkernel_
        FtMode, // Modo de interaccion para flotantes:ftmode_
        lamsps, //Lamps: lamps_
        TDensity, //Tipos de término de difusión de la densidad.: tdensity_
        ShiftingMode, //Tipos de desplazamiento aplicados a las partículas fluidas.: shiftmode_
        0, //Viscob_
        0, //Viscof_
        0, //bsbound_
        0, //bsfluid_
        ...
        0, // npbok_
        0, //id_
        divdatag, //Datos para la division celular: divdatag_
        NULL, // *dcell_
        NULL, // *posxy_
        NULL, // *posz_
        NULL, // *poscell_
        ...
        NULL, // *shiftposfs_
        NULL, // *stm_
        &kerinfo //Informacion del kernel
        );
...
//==============================================================================
/// Adjusts variables of particles of floating bodies. / Ajusta las variables de las partículas de los cuerpos flotantes.
//==============================================================================
void JSphGpu::InitFloating(){
  ...
  //-Copies massp values to GPU.
  {
    ...
    hipMemcpy(FtoMasspg,massp,sizeof(float)*FtCount,hipMemcpyHostToDevice); // Cambio: cudaMemcpy -> hipMemcpy
    delete[] massp;
  }
  //-Copies floating values to GPU.
  {
    ...
    hipMemcpy(FtoDatpg       ,datp  ,sizeof(float4)   *FtCount  ,hipMemcpyHostToDevice); // Cambio: cudaMemcpy -> hipMemcpy
    ...
    hipMemcpy(FtoVelg        ,vel   ,sizeof(float3)   *FtCount  ,hipMemcpyHostToDevice);
    hipMemcpy(FtoOmegag      ,omega ,sizeof(float3)   *FtCount  ,hipMemcpyHostToDevice);
  ...
  if(UseDEM){ //(DEM)
    ...
    hipMemcpy(DemDatag,data,sizeof(float4)*DemDataSize,hipMemcpyHostToDevice); // Cambio: cudaMemcpy -> hipMemcpy
    delete[] data;
  }
}
...
void JSphGpu::InitRunGpu(){
  ...
  if(TStep==STEP_Verlet)hipMemcpy(VelrhopM1g,Velrhopg,sizeof(float4)*Np,hipMemcpyDeviceToDevice); // Cambio: cudaMemcpy -> hipMemcpy
  if(TVisco==VISCO_LaminarSPS)hipMemset(SpsTaug,0,sizeof(tsymatrix3f)*Np); // Cambio: cudaMemset -> hipMemset
  if(CaseNfloat)InitFloating();
  if(MotionVelg)hipMemset(MotionVelg,0,sizeof(float3)*Np); //<vs_mddbc> // Cambio: cudaMemset -> hipMemset
  Check_HipError("Failed initializing variables for execution."); // Cambio: Check_CudaErroor -> Check_HipError
}
...
void JSphGpu::PreInteractionVars_Forces(unsigned np,unsigned npb){
  ...
  hipMemset(ViscDtg,0,sizeof(float)*np);                                //ViscDtg[]=0 // Cambio: cudaMemset -> hipMemset
  hipMemset(Arg,0,sizeof(float)*np);                                    //Arg[]=0 // Cambio: cudaMemset -> hipMemset
  if(Deltag)hipMemset(Deltag,0,sizeof(float)*np);                       //Deltag[]=0 // Cambio: cudaMemset -> hipMemset
  hipMemset(Aceg,0,sizeof(tfloat3)*np);                                 //Aceg[]=(0,0,0) // Cambio: cudaMemset -> hipMemset
  if(SpsGradvelg)hipMemset(SpsGradvelg+npb,0,sizeof(tsymatrix3f)*npf);  //SpsGradvelg[]=(0,0,0,0,0,0) // Cambio: cudaMemset -> hipMemset.
...
void JSphGpu::PreInteraction_Forces(){
  ...
  //-Assign memory. / Asignacion de memoria
  ...
  //-Initialise arrays. / Inicializando arrays
  ...
  VelMax=sqrt(velmax);
  hipMemset(ViscDtg,0,sizeof(float)*Np);           // Cambio: cudaMemset -> hipMemset
  ViscDtMax=0;
  Check_HipError("Failed calculating VelMax.");  // Cambio: Check_CudaErroor -> Check_HipError
...
void JSphGpu::ComputeSymplecticPre(double dt){
  ...
  hipMemcpy(Posxyg,PosxyPreg,sizeof(double2)*Npb,hipMemcpyDeviceToDevice); // Cambio: cudaMemcpy -> hipMemcpy
  ...
  ,const double2 *posxyg,const double *poszg,const unsigned *idpg,const float3 *boundnormalg)
{
  //-Allocates memory.
  unsigned n=npb;
  tdouble3 *pos=fhip::ToHostPosd3(0,n,posxyg,poszg);
  unsigned *idp=fhip::ToHostUint(0,n,idpg);
  tfloat3  *nor=fhip::ToHostFloat3(0,n,boundnormalg);
  //-Generates VTK file.
 ...
 ,const float4 *velrhopg)const
{
  ...
  tdouble3 *posh=fhip::ToHostPosd3(pini,np,posxyg,poszg);
  #ifdef CODE_SIZE4
    typecode *codeh=fhip::ToHostUint(pini,np,codeg);
  #else
    typecode *codeh=fhip::ToHostWord(pini,np,codeg);
  #endif
  unsigned *idph=fhip::ToHostUint(pini,np,idpg);
  tfloat4  *velrhoph=fhip::ToHostFloat4(pini,np,velrhopg);
  //-Creates VTK file.
...
void JSphGpu::DgSaveCsvParticlesGpu(std::string filename,int numfile,unsigned pini,unsigned pfin,std::string head,const float3 *posg,const unsigned *idpg,const float3 *velg,const float *rhopg,const float *arg,const float3 *aceg,const float3 *vcorrg){
  ...
  Check_HipError("Failed copying data from GPU.");
  //-Generates CSV file.
