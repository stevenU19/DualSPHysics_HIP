...
#include <algorithm>
#ifdef _WITHGPU
 #include "FunctionsHip.h" // Cambio de FunctionsCuda.h a FunctionsHip.h
#endif
...
void JGaugeSystem::Reset(){
  ...
  Gauges.clear();
 #ifdef _WITHGPU
  if(AuxMemoryg)hipFree(AuxMemoryg); AuxMemoryg=NULL; // Cambio cudaFree -> hipFree
 #endif
}
  ...
  ,const typecode *code,const unsigned *idp,const float4 *velrhop)
{
  ...
  if(!AuxMemoryg)fhip::Malloc(&AuxMemoryg,1); // Cambio de fcuda::Malloc a fhip::Malloc
  //-Compute measures.
  const unsigned ng=GetCount();
  for(unsigned cg=0;cg<ng;cg++){
    JGaugeItem* gau=Gauges[cg];
