...
/// \file JSphGpu_ker.h \brief Declares functions and HIP kernels for the Particle Interaction and System Update.
...
/// Estructura con constantes almacenadas en la memoria de la GPU para las interacciones de particulas.
typedef struct{
  unsigned nbound;
...
// Estructura recoleccion de informacion del Kernel.
typedef struct{
  int forcesbound_rg;
  int forcesbound_bs;
...
// Estructura con parametros para la interaccion de las particulas en la GPU.
typedef struct StrInterParmsg{
  ...
  //-Execution values. / Valores de ejecucion
  float viscob,viscof;
  ...
  //-Input data arrays. / Matriz de datos de entrada
  ...
  //-Output data arrays. / Matriz de datos de salida
  float *viscdt;
  float* ar;
  float3 *ace;
  float *delta;
  tsymatrix3f *gradvel;
  float4 *shiftposfs;
  //-Other values and objects. / Otros valores y objetos
  hipStream_t stm;
  StKerInfo *kerinfo;
...
/// Implements a set of functions and HIP kernels for the particle interaction and system update.
namespace cusph{
...
//-Kernels for the force calculation. // Kernels for the force calculation
